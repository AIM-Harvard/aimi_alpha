"""
    ----------------------------------------
    AIMI AIMI LungMask - post-processing utils
    ----------------------------------------
...
def nifti_to_dicomseg(sorted_base_path, processed_base_path,
  ...
  lungmask_output_folder = os.path.join(pat_dir_nifti_path, "lungmask")

  pred_lungs_nifti = os.path.join(lungmask_output_folder, pat_id + "_lungseg.nii.gz")
  pred_lobes_nifti = os.path.join(lungmask_output_folder, pat_id + "_lobeseg.nii.gz")

  dicom_seg_out_path = os.path.join(pat_dir_dicomseg_path, pat_id + "_SEG.dcm")

  bash_command = list()
  bash_command += ["itkimage2segimage"]
  bash_command += ["--inputImageList", "%s,%s"%(pred_lungs_nifti, pred_lobes_nifti)]
