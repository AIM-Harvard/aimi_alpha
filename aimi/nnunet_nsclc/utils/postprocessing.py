"""
    ----------------------------------------
    AIME nnU-Net NSCLC - post-processing utils
    ----------------------------------------
...
def numpy_to_nrrd(model_output_folder, processed_nrrd_path, pat_id,
                  ...
                  structure_list = ["Background", "Lung_cancer"]):
  ...
  Arguments:
    ...
    structure_list      : optional - list of the structures whose probability maps are stored in the 
                                     ...
                                     list for the NSCLC model (background = 0 included).
  Outputs:
    This function [...]
  """
...
def nifti_to_dicomseg(sorted_base_path, processed_base_path,
  ...
  pred_segmasks_nifti = os.path.join(processed_nifti_path, pat_id, pat_id + "_pred_nsclc.nii.gz")
